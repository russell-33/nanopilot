...
from nanopilot.metrics import collect_resume_metrics, render_resume_metrics_markdown  # noqa: E402


def build_arg_parser():
    parser = argparse.ArgumentParser(description="Collect nanopilot resume metrics from benchmark and run artifacts.")
    parser.add_argument("--benchmark-artifact", required=True, help="Path to benchmark artifact JSON.")
    parser.add_argument("--runs-root", required=True, help="Path to .nanopilot/runs root.")
