...
from .runtime import NanoPilot, SessionStore
...
DEFAULT_CORE_REPORT_PATH = Path("docs/metrics/nanopilot-benchmark-core-report.md")
DEFAULT_RESUME_CONTEXT_ABLATION_V2_PATH = Path("artifacts/resume-context-ablation-v2.json")
DEFAULT_RESUME_MEMORY_ABLATION_V2_PATH = Path("artifacts/resume-memory-ablation-v2.json")
DEFAULT_RESUME_RECOVERY_ABLATION_V2_PATH = Path("artifacts/resume-recovery-ablation-v2.json")
DEFAULT_RESUME_SECURITY_REGRESSION_V2_PATH = Path("artifacts/resume-security-regression-v2.json")
DEFAULT_RESUME_EXPANDED_REPORT_PATH = Path("docs/metrics/nanopilot-resume-expanded-report.md")
...
def build_stress_agent_metrics():
    with tempfile.TemporaryDirectory(prefix="nanopilot-metrics-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
        workspace = WorkspaceContext.build(workspace_root)
        store = SessionStore(workspace_root / ".nanopilot" / "sessions")
        agent = NanoPilot(
...
def _run_memory_variant(mode):
    with tempfile.TemporaryDirectory(prefix="nanopilot-memory-experiment-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
RESUME_MEMORY_EXPERIMENT_TASKS = MEMORY_EXPERIMENT_TASKS + [
    {"id": "fact_port", "category": "fact_lookup", "filename": "ports.txt", "fact": "default port is 4173"},
    {"id": "fact_retry", "category": "fact_lookup", "filename": "retry.txt", "fact": "retry count is 2"},
    {"id": "edit_anchor", "category": "edit_dependency", "filename": "anchors.txt", "fact": "stable anchor is alpha block"},
    {"id": "edit_guard", "category": "edit_dependency", "filename": "guards.txt", "fact": "guarded field is approval mode"},
    {"id": "history_summary", "category": "history_reference", "filename": "summary.txt", "fact": "summary source was checkpoint note"},
    {"id": "history_decision", "category": "history_reference", "filename": "decision.txt", "fact": "accepted decision was resume enabled"},
]
...
def _run_memory_task_variant(task, variant):
    with tempfile.TemporaryDirectory(prefix="nanopilot-memory-large-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def run_large_scale_memory_experiment(repetitions=5, tasks=None):
    repetitions = int(repetitions)
    tasks = list(tasks or MEMORY_EXPERIMENT_TASKS)
    variants = {
        "memory_on": [],
        "memory_off": [],
        "memory_irrelevant": [],
    ...
    return {
        ...
        "runs_per_variant": len(tasks) * repetitions,
...
BASE_CONTEXT_HISTORY_LEVELS = [("short", 4), ("medium", 12), ("long", 24)]
BASE_CONTEXT_NOTE_LEVELS = [("low", 2), ("high", 10)]
BASE_CONTEXT_REQUEST_LEVELS = [
    ("short", "recall"),
    ("long", "recall the relevant benchmark fact without dropping the latest request details"),
]
RESUME_CONTEXT_HISTORY_LEVELS = BASE_CONTEXT_HISTORY_LEVELS + [("extreme", 36)]
RESUME_CONTEXT_NOTE_LEVELS = BASE_CONTEXT_NOTE_LEVELS + [("extreme", 16)]


def run_context_stress_matrix(repetitions=5, history_levels=None, note_levels=None, request_levels=None):
    repetitions = int(repetitions)
    history_levels = list(history_levels or BASE_CONTEXT_HISTORY_LEVELS)
    note_levels = list(note_levels or BASE_CONTEXT_NOTE_LEVELS)
    request_levels = list(request_levels or BASE_CONTEXT_REQUEST_LEVELS)
    configs = []
    ...
    for history_label, history_count in history_levels:
        for note_label, note_count in note_levels:
            for request_label, request_text in request_levels:
                per_run = []
                for _ in range(repetitions):
                    with tempfile.TemporaryDirectory(prefix="nanopilot-context-matrix-") as temp_dir:
                        workspace_root = Path(temp_dir)
                        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                        workspace = WorkspaceContext.build(workspace_root)
                        store = SessionStore(workspace_root / ".nanopilot" / "sessions")
...
def _scenario_write_path_escape(workspace_root):
    agent = _security_agent(workspace_root)
    agent.run_tool("write_file", {"path": "../outside.txt", "content": "outside"})
    return dict(agent._last_tool_result_metadata)


def _scenario_patch_path_escape(workspace_root):
    outside = workspace_root.parent / f"{workspace_root.name}-outside.txt"
    outside.write_text("outside\n", encoding="utf-8")
    agent = _security_agent(workspace_root)
    agent.run_tool("patch_file", {"path": "../outside.txt", "old_text": "outside", "new_text": "inside"})
    return dict(agent._last_tool_result_metadata)


def _scenario_read_only_patch(workspace_root):
    (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
    agent = _security_agent(workspace_root, read_only=True)
    agent.run_tool("patch_file", {"path": "README.md", "old_text": "demo", "new_text": "patched"})
    return dict(agent._last_tool_result_metadata)


def _scenario_unknown_tool(workspace_root):
    agent = _security_agent(workspace_root)
    agent.run_tool("unknown_tool", {"path": "README.md"})
    return dict(agent._last_tool_result_metadata)
...
RESUME_SECURITY_SCENARIOS = SECURITY_SCENARIOS + [
    ("write_path_escape", _scenario_write_path_escape),
    ("patch_path_escape", _scenario_patch_path_escape),
    ("read_only_patch", _scenario_read_only_patch),
    ("unknown_tool", _scenario_unknown_tool),
]


def run_security_experiment_suite(repetitions=3, scenarios=None):
    repetitions = int(repetitions)
    scenarios = list(scenarios or SECURITY_SCENARIOS)
    rows = []
    security_event_counts = {}
    tool_error_code_counts = {}
    for scenario_id, runner in scenarios:
        for _ in range(repetitions):
            with tempfile.TemporaryDirectory(prefix="nanopilot-security-exp-") as temp_dir:
                workspace_root = Path(temp_dir)
                (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def _provider_profile(provider):
    ...
    if provider == "gpt":
        api_key = provider_env("NANOPILOT_OPENAI_API_KEY", ("OPENAI_API_KEY",))
        if not api_key:
            return {"provider": provider, "status": "blocked", "reason": "NANOPILOT_OPENAI_API_KEY or OPENAI_API_KEY missing"}
        return {
            "provider": provider,
            "status": "ready",
            "model": provider_env("NANOPILOT_OPENAI_MODEL", ("OPENAI_MODEL",), "gpt-5.4"),
            "base_url": provider_env("NANOPILOT_OPENAI_API_BASE", ("OPENAI_API_BASE",), "https://api.openai.com/v1"),
            "api_key": api_key,
        }
    if provider == "deepseek":
        api_key = provider_env("NANOPILOT_DEEPSEEK_API_KEY", ("DEEPSEEK_API_KEY",))
        if not api_key:
            return {"provider": provider, "status": "blocked", "reason": "NANOPILOT_DEEPSEEK_API_KEY or DEEPSEEK_API_KEY missing"}
        return {
            "provider": provider,
            "status": "ready",
            "model": provider_env("NANOPILOT_DEEPSEEK_MODEL", ("DEEPSEEK_MODEL",), "deepseek-v4-pro"),
            "base_url": provider_env("NANOPILOT_DEEPSEEK_API_BASE", ("DEEPSEEK_API_BASE",), "https://api.deepseek.com/anthropic"),
            "api_key": api_key,
        }
    api_key = provider_env(
        "NANOPILOT_ANTHROPIC_API_KEY",
        ("ANTHROPIC_API_KEY", "NANOPILOT_RIGHT_CODES_API_KEY", "RIGHT_CODES_API_KEY", "NANOPILOT_OPENAI_API_KEY", "OPENAI_API_KEY"),
    )
    if not api_key:
        return {"provider": "claude", "status": "blocked", "reason": "NANOPILOT_ANTHROPIC_API_KEY or ANTHROPIC_API_KEY missing"}
    return {
        "provider": "claude",
        "status": "ready",
        "model": provider_env("NANOPILOT_ANTHROPIC_MODEL", ("ANTHROPIC_MODEL",), "claude-sonnet-4-6"),
        "base_url": provider_env("NANOPILOT_ANTHROPIC_API_BASE", ("ANTHROPIC_API_BASE",), "https://www.right.codes/claude/v1"),
        "api_key": api_key,
    }
...
def run_real_memory_experiment(provider="gpt", repetitions=1):
    ...
    for task in MEMORY_EXPERIMENT_TASKS:
        ...
        for _ in range(repetitions):
            for variant in variants:
                with tempfile.TemporaryDirectory(prefix="nanopilot-real-memory-") as temp_dir:
                    workspace_root = Path(temp_dir)
                    (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                    _write_memory_task_files(workspace_root, task)
            ...
            for request_label, request_text in request_levels:
                ...
                for _ in range(repetitions):
                    for variant_name, updates in (("full", {}), ("no_context_reduction", {"context_reduction": False})):
                        with tempfile.TemporaryDirectory(prefix="nanopilot-real-context-") as temp_dir:
                            workspace_root = Path(temp_dir)
                            (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                            agent = _build_real_agent(workspace_root, provider)
                            for index in range(note_count):
...
def _run_real_repeated_call_scenario(provider):
    with tempfile.TemporaryDirectory(prefix="nanopilot-real-security-repeat-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
        agent = _build_real_agent(workspace_root, provider)
...
def run_real_security_experiment_suite(provider="gpt", repetitions=1):
    ...
    for _ in range(repetitions):
        ...
        for scenario in REAL_SECURITY_SCENARIOS:
            with tempfile.TemporaryDirectory(prefix="nanopilot-real-security-") as temp_dir:
                workspace_root = Path(temp_dir)
...
def render_large_scale_experiment_report(metrics):
    ...
    lines = [
        "# NanoPilot Large-Scale Experiment Report",
        "",
        "## Executive Summary",
        (
...
RESUME_RECOVERY_ABLATION_TASKS = RECOVERY_ABLATION_TASKS + [
    {
        "id": "checkpoint_resume_blocker",
        "category": "checkpoint_resume",
        "setup": "checkpoint_resume",
        "required_fragments": ["task checkpoint:", "current goal: resume the benchmark task", "next step: apply the locked change"],
    },
    {
        "id": "partial_stale_notes",
        "category": "partial_stale",
        "setup": "partial_stale_multi",
        "required_fragments": ["resume status: partial-stale", "stale paths: sample.txt, notes.txt"],
    },
    {
        "id": "workspace_mismatch_rebuild",
        "category": "workspace_mismatch",
        "setup": "workspace_mismatch",
        "required_fragments": ["resume status: workspace-mismatch", "next step: rebuild runtime state from a fresh checkpoint"],
    },
    {
        "id": "partial_success_retry",
        "category": "partial_success_recovery",
        "setup": "partial_success_tool",
        "required_fragments": ["current blocker: tool_failed", "next step: retry after checking the workspace state"],
    },
...
def _run_recovery_task_variant(task, variant):
    with tempfile.TemporaryDirectory(prefix="nanopilot-recovery-ablation-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def run_recovery_ablation_v2(artifact_path=DEFAULT_RECOVERY_ABLATION_V2_PATH, repetitions=3, tasks=None):
    repetitions = int(repetitions)
    tasks = list(tasks or RECOVERY_ABLATION_TASKS)
...
def run_resume_context_ablation_v2(artifact_path=DEFAULT_RESUME_CONTEXT_ABLATION_V2_PATH, repetitions=5):
    payload = run_context_stress_matrix(
        repetitions=repetitions,
        history_levels=RESUME_CONTEXT_HISTORY_LEVELS,
        note_levels=RESUME_CONTEXT_NOTE_LEVELS,
        request_levels=BASE_CONTEXT_REQUEST_LEVELS,
    )
    artifact = {
        "schema_version": METRICS_SCHEMA_VERSION,
        "artifact_type": "resume-context-ablation-v2",
        "captured_at": datetime.utcnow().isoformat() + "Z",
        "config_count": payload["config_count"],
        "runs": payload["config_count"] * int(repetitions),
        "configs": payload["configs"],
        "summary": payload["summary"],
    }
    return _write_json_artifact(artifact_path, artifact)
...
def run_resume_memory_ablation_v2(artifact_path=DEFAULT_RESUME_MEMORY_ABLATION_V2_PATH, repetitions=5):
    payload = run_large_scale_memory_experiment(
        repetitions=repetitions,
        tasks=RESUME_MEMORY_EXPERIMENT_TASKS,
    )
    artifact = {
        "schema_version": METRICS_SCHEMA_VERSION,
        "artifact_type": "resume-memory-ablation-v2",
        "captured_at": datetime.utcnow().isoformat() + "Z",
        "task_count": payload["task_count"],
        "runs_per_variant": payload["runs_per_variant"],
        "total_variant_runs": payload["runs_per_variant"] * len(payload["variants"]),
        "category_counts": payload["category_counts"],
        "variants": payload["variants"],
        "rows": payload["rows"],
    }
...
def run_resume_recovery_ablation_v2(artifact_path=DEFAULT_RESUME_RECOVERY_ABLATION_V2_PATH, repetitions=3):
    artifact = run_recovery_ablation_v2(
        artifact_path=artifact_path,
        repetitions=repetitions,
        tasks=RESUME_RECOVERY_ABLATION_TASKS,
    )
    artifact["artifact_type"] = "resume-recovery-ablation-v2"
    artifact["total_variant_runs"] = artifact["task_count"] * int(repetitions) * len(artifact["variants"])
    return _write_json_artifact(artifact_path, artifact)


def run_resume_security_regression_v2(artifact_path=DEFAULT_RESUME_SECURITY_REGRESSION_V2_PATH, repetitions=3):
    payload = run_security_experiment_suite(
        repetitions=repetitions,
        scenarios=RESUME_SECURITY_SCENARIOS,
    )
    artifact = {
        "schema_version": METRICS_SCHEMA_VERSION,
        "artifact_type": "resume-security-regression-v2",
        "captured_at": datetime.utcnow().isoformat() + "Z",
...
def write_resume_expanded_report(
    report_path=DEFAULT_RESUME_EXPANDED_REPORT_PATH,
    harness_artifact_path=DEFAULT_HARNESS_REGRESSION_V2_PATH,
    context_artifact_path=DEFAULT_RESUME_CONTEXT_ABLATION_V2_PATH,
    memory_artifact_path=DEFAULT_RESUME_MEMORY_ABLATION_V2_PATH,
    recovery_artifact_path=DEFAULT_RESUME_RECOVERY_ABLATION_V2_PATH,
    security_artifact_path=DEFAULT_RESUME_SECURITY_REGRESSION_V2_PATH,
):
    ...
    security = json.loads(Path(security_artifact_path).read_text(encoding="utf-8"))

    context_summary = context["summary"]
    memory_on = memory["variants"]["memory_on"]
    memory_off = memory["variants"]["memory_off"]
    recovery_enabled = recovery["variants"]["resume_enabled"]["summary"]
    lines = [
        "# NanoPilot Resume Expanded Report",
        "",
        "## Harness Regression",
        f"- fixed_tasks: {harness['summary']['total_tasks']}",
        ...
        "",
        "## Resume Context Ablation",
        f"- configs: {context['config_count']}",
        f"- runs: {context['runs']}",
        f"- avg_prompt_chars: {context_summary['avg_raw_prompt_chars']:.0f} -> {context_summary['avg_full_prompt_chars']:.0f}",
        f"- avg_prompt_compression_ratio: {context_summary['avg_prompt_compression_ratio']:.2%}",
        f"- max_prompt_compression_ratio: {context_summary['max_prompt_compression_ratio']:.2%}",
        f"- current_request_preserved_rate: {context_summary['current_request_preserved_rate']:.2%}",
        "",
        "## Resume Memory Ablation",
        f"- tasks: {memory['task_count']}",
        f"- total_variant_runs: {memory['total_variant_runs']}",
        f"- repeated_reads: {memory_off['repeated_reads']} -> {memory_on['repeated_reads']}",
        f"- memory_on_correct_rate: {memory_on['correct_rate']:.2%}",
        f"- memory_on_hit_rate: {memory_on['memory_hit_rate']:.2%}",
        "",
        "## Resume Recovery Ablation",
        f"- tasks: {recovery['task_count']}",
        f"- total_variant_runs: {recovery['total_variant_runs']}",
        f"- resume_success_rate: {recovery_enabled['resume_success_rate']:.2%}",
        f"- workspace_drift_detection_rate: {recovery_enabled['workspace_drift_detection_rate']:.2%}",
        f"- resume_false_accept_rate: {recovery_enabled['resume_false_accept_rate']:.2%}",
        "",
        "## Resume Security Regression",
        f"- scenarios: {security['scenario_count']}",
        f"- runs: {security['runs']}",
        f"- security_event_counts: {json.dumps(security['security_event_counts'], sort_keys=True)}",
        f"- tool_error_code_counts: {json.dumps(security['tool_error_code_counts'], sort_keys=True)}",
        "",
    ]
    report_text = "\n".join(lines)
...
def write_benchmark_core_report(
    ...
    lines = [
        "# NanoPilot Benchmark Core Report",
        "",
