...
NanoPilot 就是包在模型外面的控制循环：负责组 prompt、解析模型输出、
...
class NanoPilot:
    def __init__(
        ...
        self.run_store = run_store or RunStore(Path(workspace.repo_root) / ".nanopilot" / "runs")
    ...
    def build_prefix(self):
        ...
        text = textwrap.dedent(
            f"""\
            You are NanoPilot, a small local coding agent working inside a local repository.
    ...
    def ask(self, user_message):
        ...
        如果新人想理解 nanopilot 是怎么“从一句话跑成一个 agent 流程”的，
        这里就是最关键的入口。
        """
    ...
    def parse(raw):
        ...
        if "<tool>" in raw and ("<final>" not in raw or raw.find("<tool>") < raw.find("<final>")):
            body = NanoPilot.extract(raw, "tool")
            try:
                payload = json.loads(body)
            except Exception:
                return "retry", NanoPilot.retry_notice("model returned malformed tool JSON")
            if not isinstance(payload, dict):
                return "retry", NanoPilot.retry_notice("tool payload must be a JSON object")
            if not str(payload.get("name", "")).strip():
                return "retry", NanoPilot.retry_notice("tool payload is missing a tool name")
            args = payload.get("args", {})
            if args is None:
                payload["args"] = {}
            elif not isinstance(args, dict):
                return "retry", NanoPilot.retry_notice()
            return "tool", payload
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            payload = NanoPilot.parse_xml_tool(raw)
            if payload is not None:
                return "tool", payload
            return "retry", NanoPilot.retry_notice()
        if "<final>" in raw:
            final = NanoPilot.extract(raw, "final").strip()
            if final:
                return "final", final
            return "retry", NanoPilot.retry_notice("model returned an empty <final> answer")
        raw = raw.strip()
        if raw:
            return "final", raw
        return "retry", NanoPilot.retry_notice("model returned an empty response")
    ...
    def parse_xml_tool(raw):
        ...
        if not match:
            return None
        attrs = NanoPilot.parse_attrs(match.group("attrs"))
        ...
        for key in ("content", "old_text", "new_text", "command", "task", "pattern", "path"):
            if f"<{key}>" in body:
                args[key] = NanoPilot.extract_raw(body, key)
